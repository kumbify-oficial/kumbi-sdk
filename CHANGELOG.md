# kumbi-sdk

## 1.7.1

### Patch Changes

- update oauth client profile api

## 1.7.0

### Minor Changes

- add profile creation to oauth client

## 1.6.6

### Patch Changes

- update kwaba auth

## 1.6.5

### Patch Changes

- update waba retrieve

## 1.6.4

### Patch Changes

- refactor auth clients

## 1.6.3

### Patch Changes

- update kwaba client apikey to accessToke

## 1.6.2

### Patch Changes

- return integration info

## 1.6.1

### Patch Changes

- update social clients

## 1.6.0

### Minor Changes

- fix-bug auth headers

## 1.5.14

### Patch Changes

- fix-bug kwaba generate qrcode

## 1.5.13

### Patch Changes

- add waba qrcode to genere by app

## 1.5.12

### Patch Changes

- add revokeAcessToken to oauth client

## 1.5.11

### Patch Changes

- update koauth add serviceInfo

## 1.5.10

### Patch Changes

- update koauth generate token

## 1.5.9

### Patch Changes

- fix-bug: koauth

## 1.5.8

### Patch Changes

- update koauth constructor

## 1.5.7

### Patch Changes

- update koaut client

## 1.5.6

### Patch Changes

- add idempodency

## 1.5.5

### Patch Changes

- feat: add fb scopes

## 1.5.4

### Patch Changes

- update fb and insta clients

## 1.5.3

### Patch Changes

- remove kwaba dev test

## 1.5.2

### Patch Changes

- update package.json type module

## 1.5.1

### Patch Changes

- update oauth scopes

## 1.5.0

### Minor Changes

- add new clients

## 1.4.0

### Minor Changes

- feat: add paypay angolan method

## 1.3.5

### Patch Changes

- update client constructor types

## 1.3.4

### Patch Changes

- fix-bug: update oauth client

## 1.3.3

### Patch Changes

- fix-bug: retriev correct user oauth client token

## 1.3.2

### Patch Changes

- add phone param to oauth client

## 1.3.1

### Patch Changes

- update payment client types

## 1.3.0

### Minor Changes

- update payment client and transpilation

## 1.2.5

### Patch Changes

- rebuild package

## 1.2.4

### Patch Changes

- refactor payment client

## 1.2.3

### Patch Changes

- update payment clients

## 1.2.2

### Patch Changes

- resolve stripe payment
- resolve crypto payment

## 1.2.1

### Patch Changes

- resolve payment client

## 1.2.0

### Minor Changes

- Add KPaymentClient

## 1.1.6

### Patch Changes

- update readme

## 1.1.5

### Patch Changes

- resolve maps

## 1.1.4

### Patch Changes

- resolve readme documentation

## 1.1.3

### Patch Changes

- resolve oauth2client generateUrls lang

## 1.1.2

### Patch Changes

- Resolve oauthclient scopes

## 1.1.1

### Patch Changes

- Resolve maps

## 1.1.0

### Minor Changes

- Add oatuh2client and api context

## 1.0.11

### Patch Changes

- feat: add services return types

## 1.0.10

### Patch Changes

- update README

## 1.0.9

### Patch Changes

- update README, sms to recipients

## 1.0.8

### Patch Changes

- update docs

## 1.0.7

### Patch Changes

- update docs, add mail template send

## 1.0.6

### Patch Changes

- fix-bug: send mail, no args found

## 1.0.5

### Patch Changes

- update client exceptions

## 1.0.4

### Patch Changes

- update clients exceptions

## 1.0.3

### Patch Changes

- add provider clients and README

## 1.0.2

### Patch Changes

- update dist files and uniform types

## 1.0.1

### Patch Changes

- initial package

## 2.0.3

### Patch Changes

- update server sms and mail

## 2.0.2

### Patch Changes

- update current servers

## 2.0.1

### Patch Changes

- update mail server

## 2.0.0

### Major Changes

- Initial package
